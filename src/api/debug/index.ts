  ...
  if (options.sep) {
    ...
    if (currentCommand) {
      ...
      const debugConfig = {
        ...
        "startBatchJobCommand": `SBMJOB CMD(${currentCommand}) INLLIBL(${options.libraries.libraryList.join(` `)}) CURLIB(${options.libraries.currentLibrary}) JOBQ(${config?.batchDebugJOBQ}) MSGQ(*USRPRF) CPYENVVAR(*YES)`,
